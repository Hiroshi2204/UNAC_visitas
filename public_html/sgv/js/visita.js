...
function VerificarVisitante(){
    ...
    if(nrodoc.length==0) {
        return Swal.fire("Mensaje de Advertencia","Llene el Número de Documento","warning");
    }
	...
	$.ajax({
		...
		if(resp==0){
			...
			$("#txt_materno").val("");
			$("#txt_nombres").val("");
			$("#txt_paterno").prop( "disabled", false );$("#txt_materno").prop( "disabled", false );$("#txt_nombres").prop( "disabled", false );
		}else{
			var data= JSON.parse(resp);
			$("#txt_paterno").val(data[0]['apepat']);
			$("#txt_materno").val(data[0]['apemat']);
			$("#txt_nombres").val(data[0]['nombres']);
			$("#txt_paterno").prop( "disabled", true );$("#txt_materno").prop( "disabled", true );$("#txt_nombres").prop( "disabled", true );
		}
...
function VerificarRuc(){
    ...
    if(ruc.length==0) {
        return Swal.fire("Mensaje de Advertencia","Llene el Número de RUC","warning");
    }
	
	if(ruc.length!=11) {
        return Swal.fire("Mensaje de Advertencia","Número de RUC incorrecto","warning");
    }
...
function listar_visita(xsede){
       ...
       "columns":[
		   ...
		   {"data":"motivo"},
		   {"data":"ingreso"},
           {"defaultContent":"<button style='font-size:13px;' type='button' class='finalizar btn btn-danger' \"><i class='fa fa-close'></i></button>"}
       ],
...
function Registrar_Visita(){
	...
	if(xtipo!="1" && (xruc.length==0 || xrazon.length==0)){
		return Swal.fire("Mensaje de Advertencia","Ingrese Ruc y Razón Social","warning");
	}
	...
	if($("#chk_otro").is(':checked')){
		...
		if(cboficina_id=="0"){return Swal.fire("Mensaje de Advertencia","Seleccione Oficina y Funcionario","warning");}
	...
	if(xtipodoc.length==0 || xdoc.length==0 || xpaterno.length==0 || xmaterno.length==0 || xnombres.length==0 || xtipo.length==0 || xmotivo.length==0 || xarea.length==0 || xfuncionario.length==0){
	...
	$.ajax({
		...
		if(resp>0){
			if(resp==1){
				...
				Swal.fire("Mensaje de Confirmación","Datos correctamente, Nueva Visita Registrada","success")
				.then((value)=>{
					LimpiarRegistro()
					table.ajax.reload();
				});
...
function listar_combo_oficinas(){
	$.ajax({
		...
		//alert(resp);
		//alert(content.data.orgchart[0].responsable);
		//alert(content.data.orgchart.length)
		var cadena="";
		if(content.data.orgchart.length>0){
			for(var i=0; i<content.data.orgchart.length; i++){
				cadena +="<option value='"+content.data.orgchart[i].bpId+"|"+content.data.orgchart[i].roleId+"|"+content.data.orgchart[i].areaId+"'>"+content.data.orgchart[i].area+" | "+content.data.orgchart[i].role + " | "+ content.data.orgchart[i].responsable+"</option>";
			}
			$("#cb_oficina").html($("#cb_oficina").html()+cadena);
		}else{
			cadena +="<option value ''> NO SE ENCONTRARON REGISTROS</option>";
		}
})
